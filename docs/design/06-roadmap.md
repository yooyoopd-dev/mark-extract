...
| 항목 | 왜 정해야 하나 |
|---|---|
| **llm-co-wiki 차용 범위** | 재라이선스 전제가 성립하지 않음 → [ATTRIBUTION](ATTRIBUTION.md#llm-co-wiki--gpl-결정-보류) |
| **Mark Extract 자체 라이선스** | 배포 형태와 위 항목에 걸림 |
| 코드 서명 | 미서명 시 SmartScreen·사내 정책에 걸릴 수 있음 |
| 자동 업데이트 | 현재 설계는 없음. 오프라인 전제와 충돌 |
| 변환 이력 보존 기간 | `queue.json`이 무한히 커지는 것을 막을 정책 |
| 대용량 문서 상한 | 현재 500MB 가정 |
| LLM 프롬프트 사내 검수 | 프롬프트 전문을 설정에서 열람 가능하게 했으나 승인 절차는 미정 |
| PPTX 확장 범위 | SmartArt·OLE 객체를 언제 다룰지 |
