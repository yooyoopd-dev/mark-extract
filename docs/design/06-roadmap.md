...
프로바이더·모델·모드 기본값, Ollama 모델 드롭다운(`/api/tags`), CLI 진단 리포트, 프롬프트 전문 열람, 감시 폴더 관리, 테마.

**검증**
- Ollama HTTP가 응답하지 않아도 CLI 단독으로 동작하고 "잘림을 감지할 수 없음" 경고가 뜬다
- 진단 리포트를 복사할 수 있다
- 프롬프트 전문이 실제로 쓰이는 것과 같다
