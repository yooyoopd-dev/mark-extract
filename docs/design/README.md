...
| 12 | 출력 | Markdown만 |
| 13 | 언어 | 한국어·영어 i18n |
| 14 | llm-co-wiki 차용 | **사실 정보만 인용, 코드 복사 없음.** 저장소 public 유지 — 근거는 [ATTRIBUTION.md](ATTRIBUTION.md) |
...
| 24 | PPTX 범위 | 텍스트·표·노트·이미지·차트. SmartArt·OLE·애니메이션은 건너뜀 |
| 25 | LLM 프롬프트 검수 | 앱은 전문 열람·복사만 제공. 승인 절차는 조직 소관 |

결정 18의 의미: 저장소는 public이라 소스가 보이지만 `LICENSE` 파일이 없어 기본 저작권이 유지된다. 열람은 되고 사용 허가는 부여되지 않는다. 동봉하는 Apache-2.0·MIT 구성요소의 고지 의무는 그대로다.
