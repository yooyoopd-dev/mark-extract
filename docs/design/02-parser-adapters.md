...
- 프로세스 제한 시간은 **설정(`localTimeoutMin`, 기본 30분)** 이 정하고, **OCR 을 켜면 30분이 최소 보장선**이다(설정이 더 크면 설정값). `--hybrid-timeout` 은 백엔드 쪽 시간이라 건드리지 않는다(0 = 백엔드 기본값)

> **build.28 실측 — 10분이 정상 동작을 끊었다.** 로컬 기본이 10분이었고 OCR 을 켠
> 스캔 문서가 그 안에 끝나지 않았다. 30분으로 올리고 설정에서 늘릴 수 있게 했다.
>
> 로그도 같이 고쳤다. 전에는 `제한 시간 30분` 이 **문구로 박혀** 있어서, 실제로는
> 문서에 실린 값이 쓰이는데도 로그만 보고는 몇 분에 끊긴 것인지 알 수 없었다.
> 이제 `제한 시간` 항목에 실제로 쓴 값을 적는다.
>
> 큐가 새 문서에 심는 값도 갈랐다. 전에는 로컬 문서에도 LLM 제한 시간
> (`llmTimeoutMin`)을 심어, 설정 화면이 "로컬은 10분 고정" 이라 적어 둔 것과
> 실제가 어긋나 있었다 — 어느 쪽 숫자가 쓰이는지 아무도 몰랐다.
