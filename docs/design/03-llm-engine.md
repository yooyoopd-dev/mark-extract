...
**셰임은 cmd 파서를 두 번 지나간다.** cmd.exe가 한 번, 배치 파일 자신의 `%*` 확장이 또 한 번. Node가 배치 파일 spawn을 아예 막은 이유가 이것이다(CVE-2024-24576과 같은 뿌리). 무엇이 그 왕복을 견디는지는 Windows CI에서 **글자마다 따로 쟀다**.

| 인자 | 왕복 |
|---|---|
| `qwen2.5:14b` · 공백 · 한글 · `C:\Temp\문서 폴더\x.txt` | 통과 |
| `&` `\|` `<>` `^` `"` `%` `!` 각각 하나씩 | 통과 |
| `a&b c"d^e\|f` (한 인자에 여럿) | **깨진다** |

실제 모델 이름은 `[A-Za-z0-9.:_-]` 범위이고 경로는 우리가 만든다. 그래서 **여럿을 섞은 경우는 막지 않고 한계로 남긴다** — 어느 글자가 방아쇠인지 확인하지 않았고, 확인하지 않은 것을 근거로 입력을 거절하면 멀쩡한 이름까지 막을 수 있다.
