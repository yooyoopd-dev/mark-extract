...
**정상 위치에서 다시 재고 정했다** (run 22·23):

| | 1회차 | 2회차 |
|---|---|---|
| 기본 (7z) | 20.5~23.9초 | 17.5~21.4초 |
| **`useZip`** | 21.8~26.3초 | **12.7~12.8초** |

**1회차는 개선되지 않는다.** exe 가 173MB → 264.8MB 로 커져 처음 읽는 비용이 는 것이
압축 해제에서 번 것을 상쇄한 것으로 보인다(추정 — 어느 쪽이 얼마인지 가르지 않았다).
**2회차는 17.5~21.4초에서 12.7~12.8초로 줄었다.** 파일 캐시가 더워진 뒤에는 zlib 해제와
사라진 복사가 그대로 드러난다.
